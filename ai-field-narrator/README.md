...
Speech transcription, word-level timestamp mapping, and field-derivation playback — the original project in this repo, now living under its own folder.
...
### 1. Navigate to this project's backend:

   ```bash
   cd ai-field-narrator/backend
   ```
...
Create a `.env` file in `ai-field-narrator/backend/` and add your OpenAI API key and Google Cloud Vertex configuration:
```bash
...
From `ai-field-narrator/backend/`, run:
```bash
python3 main.py
```
